...
from PIL import ImageFilter # må bruke
...
class Artwork:
    ...
    def func1(self,img1, img2,levels,scale):
        img1.rotate(180)
        img1 = img1.tunnel(levels=levels,scale=scale) #cropper bildet som rammer inni hverandre
        img3 = img1.mortun(img2,levels=1,scale=scale) #firedobler bildet og lager smooth overgang
        return img3

    #endre farger
    def func2(self,img1,img2):
        img3 = img1.map_color_wta() #Gjør bildene røde og blå i kontraster
        img3 = img3.morph4(img2) #firedobler bildet til collage
        return img3

    #fikser farger og kaller func1
    def func3(self,img1,img2):
        img1 = img1.map_color_wta()
        img3 = self.func1(img1,img2,3,0.9)
        return img3


    #fikser alle bildene ferdig
    def changeImages(self):
        a = self.func2(self.img1,self.img2)
        b = self.func1(a.resize(self.size, self.size),self.img3,4,0.9)
        c = self.func3(a,b)
        c.rotate(90)
        d = self.func2(c,c)
        d.display()
